...
function buildSort(sort) {
  // allow: price_asc, price_desc, newest, rating_desc, stock_desc
  switch (sort) {
    case 'price_asc':
...
async function listProducts(query) {
  ...
  // If $text search is used, you can also project textScore
...
async function updateProduct({ productId, requester, body }) {
  ...
  // vendor can only modify own products; admin can modify any
...
async function addReview({ productId, requester, body }) {
  ...
  p.ratingCount = count;
  p.ratingAvg = Math.round(avg * 10) / 10; // 1 decimal
